...
| `api_tests.py` | Python 3 (stdlib) | Behavioural matrix with an independent HTTP client: lifecycle, visibility timeout, abandon→redelivery, DLQ+redrive, explicit dead-letter, defer, schedule, dedup window + conflict, session ordering, receive-and-delete, fencing; managed-key producer/consumer separation, pagination, denial and revocation. |
